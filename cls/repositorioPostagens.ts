...
export class RepositorioDePostagens {
    ...
    consultar(id: number, hashtag: string, texto?: string,  perfil?: Perfil): Postagem[] {
        ...
        if (!profileFound && id != -1) {
            throw new PerfilSemPostagemRegularError("\nERRO: perfil não possui postagens regulares...")
        }
        ...
        if (posts.length == 0) {
            throw new HashtagInexistenteError("\nERRO: não foram encontrados postagens com essa hashtag...")
        }
        return posts
    }
    ...
    consultarUnico(postId: number): Postagem {
        ...
        // A postagem precisa ser avançada
        if (thePost[0] instanceof PostagemAvancada) {
            return thePost[0]
        }
        else {
            throw new PostagemRegularError("\nERRO: somente postagens avançadas podem ser curtidas... [repositorioPostagens.ts/consultarUnico]")
        }
    }
    ...
    consultarPorIdPerfil(id: number): PostagemAvancada[] {
        ...
        for(let i = 0; i < this.postagens.length; i++) {
            const currentPost: Postagem = this.postagens[i]
            console.log("--o", currentPost.perfil.id, id, currentPost.perfil.id === id)
        ...
        if (!fromId) {
            throw new PerfilSemPostagemAvancadaError("\nERRO: perfil não possui postagens avançadas...")
        }
        return posts
    }
